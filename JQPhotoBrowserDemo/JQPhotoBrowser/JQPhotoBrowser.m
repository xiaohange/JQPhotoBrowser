...
#import "JQPhotoItemView.h"

#define kPadding 20


@interface JQPhotoBrowser()<UIScrollViewDelegate,UIGestureRecognizerDelegate,JQPhotoItemViewDelegate,UIActionSheetDelegate>
...
@property (nonatomic, strong) UILabel *topPageCtrLabel;
...
- (UILabel *)topPageCtrLabel
{
    if (!_topPageCtrLabel) {
        _topPageCtrLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 100)/2, 32, 100, 20)];
        _topPageCtrLabel.textColor = [UIColor whiteColor];
        _topPageCtrLabel.font = [UIFont systemFontOfSize:14];
        _topPageCtrLabel.textAlignment = NSTextAlignmentCenter;
        [_contentView addSubview:_topPageCtrLabel];
    }
    return _topPageCtrLabel;
}
...
- (void)showFromImageView:(UIView *)fromView andCurrentIndex:(NSInteger)currentIndex completion:(void (^)(void))completion
{
    ...
    if (_shouldShowPageControl) {
        ...
        if (_pageType == bottomPageCtrType) {
            self.pager.alpha = 0;
            self.pager.numberOfPages = self.groupItems.count;
            self.pager.currentPage = currentIndex;
        }else
        {
            self.topPageCtrLabel.text = [NSString stringWithFormat:@"%zd/%zd",currentIndex+1,self.groupItems.count];
        }
    }
    
    _scrollView.contentSize = CGSizeMake(_scrollView.frame.size.width * self.groupItems.count, _scrollView.frame.size.height);
    [_scrollView scrollRectToVisible:CGRectMake(_scrollView.frame.size.width * currentIndex, 0, _scrollView.frame.size.width, _scrollView.frame.size.height) animated:NO];
    [self scrollViewDidScroll:_scrollView];
    
    [UIView setAnimationsEnabled:YES];
    
    
    JQPhotoItemView *cell = [self cellForPage:self.currentPage];
    JQPhotoItem *item = _groupItems[self.currentPage];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            JQPhotoItemView *cell = [self cellForPage:i];
            if (!cell) {
                JQPhotoItemView *cell = [self dequeueReusableCell];
    ...
    if (_shouldShowPageControl) {
        ...
        {
            self.topPageCtrLabel.text = [NSString stringWithFormat:@"%zd/%zd",intPage+1,_groupItems.count];
        }
    }
...
#pragma mark ----- JQPhotoItemViewDelegate
- (void)zoomEnd
{
    _isPresented = YES;
    [self scrollViewDidScroll:_scrollView];
...
- (void)updateCellsForReuse {
    for (JQPhotoItemView *cell in _cells) {
        if (cell.superview) {
...
- (JQPhotoItemView *)dequeueReusableCell {
    JQPhotoItemView *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (JQPhotoItemView *)cellForPage:(NSInteger)page {
    for (JQPhotoItemView *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)gesture
{
    ...
    JQPhotoItemView *itemView = [self cellForPage:self.currentPage];
    if (itemView) {
...
- (void)longPress:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        JQPhotoItemView *photoItem = [self cellForPage:self.currentPage];
        if (!photoItem.imageView.image) return;
...
- (void)dismissAnimated:(BOOL)animated completion:(void(^)(void))completion
{
    ...
    JQPhotoItemView *cell = [self cellForPage:currentPage];
    JQPhotoItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(JQPhotoItemView *cell, NSUInteger idx, BOOL *stop) {
